...
export function AuthModal({ isOpen, onClose, initialView = 'signin' }: AuthModalProps) {
  const { signIn, signUp } = useAuth();  // ← updated from login
      ...
      >
        ...
        <div className="px-8 py-6">
          ...
          <div className="grid grid-cols-2 gap-3 mb-5">
            <button type="button" className="flex justify-center items-center gap-2 py-2.5 px-4 border border-white/10 rounded-xl bg-[#0A0A0A] text-sm font-medium text-white hover:bg-white/5 hover:border-white/20 transition-all">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div>
              ...
              <input
                ...
                className={`block w-full rounded-xl bg-[#0A0A0A] border px-4 py-2.5 text-white placeholder-zinc-600 focus:outline-none focus:ring-1 text-sm transition-colors ${
                  emailError ? 'border-red-500/70 focus:border-red-500 focus:ring-red-500' : 'border-white/10 focus:border-indigo-500 focus:ring-indigo-500'
                }`}
            ...
            <div>
              ...
              <input
                ...
                className={`block w-full rounded-xl bg-[#0A0A0A] border px-4 py-2.5 text-white placeholder-zinc-600 focus:outline-none focus:ring-1 text-sm transition-colors ${
                  passwordError ? 'border-red-500/70 focus:border-red-500 focus:ring-red-500' : 'border-white/10 focus:border-indigo-500 focus:ring-indigo-500'
                }`}
                placeholder="••••••••"
              />
            ...
            {view === 'signin' && (
              <div className="flex items-center gap-2">
                <input id="remember-me" type="checkbox" className="h-4 w-4 rounded border-white/10 bg-[#0A0A0A] text-indigo-500 focus:ring-indigo-500 focus:ring-offset-[#141414]" />
                <label htmlFor="remember-me" className="text-sm text-zinc-400 select-none cursor-pointer">Remember me</label>
              </div>
            )}

            {/* ← Supabase error shown here */}
            {authError && (
              <p className="text-sm text-red-400 text-center py-1">{authError}</p>
            )}

            <button
              type="submit"
              disabled={loading}
              className="w-full flex justify-center items-center py-2.5 px-4 rounded-full text-sm font-semibold text-black bg-white hover:bg-zinc-200 active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-[#141414] focus:ring-white transition-all mt-1 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading
                ? 'Please wait...'
