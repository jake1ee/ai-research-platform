...
Sends each model's output to a separate "judge" LLM that rates the quality
of the answer on a 0–10 scale and provides reasoning.

Protocol
--------
  The judge is asked to return a JSON object:
    { "score": <float 0-10>, "reasoning": "<short explanation>" }

  We use structured JSON output where the judge model supports it, and fall
  back to regex extraction otherwise.
...
-------
    ...
    scores = await score_with_judge(
        ...
        outputs={
            "gpt-4o": "Recursion is a function calling itself ...",
            "claude-3-5-sonnet": "A technique where a function ...",
        },
        judge_model="openai/gpt-4o",
    )
    for model_id, result in scores.items():
        print(model_id, result.score, result.reasoning)
"""
...
def _parse_judge_response(text: str) -> tuple[Optional[float], Optional[str]]:
    """
    Extract (score, reasoning) from the judge's text output.
    ...
    # Attempt 2: JSON embedded in prose / code fence
    ...
    # Attempt 3: regex extraction – "score": 7.5
...
async def _score_single(
    ...
    """Score one model's output using the judge LLM."""
    ...
    try:
        response = await acompletion(
            ...
            temperature=0,  # Judge should be deterministic
...
async def score_with_judge(
    prompt: str,
    outputs: Dict[str, str],         # { model_id: output_text }
    ...
    Parameters
    ----------
    prompt      : The original user prompt (context for the judge).
    outputs     : Dict mapping model_id → raw text output.
    judge_model : LiteLLM model string for the judge. Defaults to config value.

    Returns
    -------
    Dict mapping model_id → JudgeScore (score + reasoning).
    Models that errored during judging have JudgeScore.error set.
    """
    judge = judge_model or f"openai/{settings.DEFAULT_JUDGE_MODEL}"

    # Filter out models that produced no output (failed calls)
    ...
    # Models that were skipped (no output) get an error entry
