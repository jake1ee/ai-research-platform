...
import { LayoutGrid, Eye, EyeOff, Loader2, Check } from 'lucide-react'
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await withMinDuration(() => signUp(email, password, name))
  ...
  return (
    <div className="min-h-screen bg-zinc-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans">
      <div className="sm:mx-auto sm:w-full sm:max-w-md flex flex-col items-center">
        ...
        <h2 className="text-center text-3xl font-bold tracking-tight text-zinc-900">Create your account</h2>
        ...
        <div className="bg-white py-8 px-4 shadow-sm border border-zinc-200 sm:rounded-3xl sm:px-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {password.length > 0 && (
                <div className="mt-2 space-y-1">
                  <div className="flex gap-1">
                    {[1, 2, 3].map(i => (
                      <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i <= passwordStrength ? strengthColor : 'bg-zinc-200'}`} />
                    ))}
                  </div>
            ...
            <p className="text-center text-xs text-zinc-500">
              ...
              <a href="#" className="underline hover:text-zinc-700">Terms</a> and{' '}
              <a href="#" className="underline hover:text-zinc-700">Privacy Policy</a>.
            </p>
          </form>

          {/* Benefits */}
          <ul className="mt-6 space-y-2 border-t border-zinc-100 pt-5">
            {['No credit card required', 'Full access for 7 days', 'Cancel anytime'].map(benefit => (
              <li key={benefit} className="flex items-center gap-2 text-xs text-zinc-500">
